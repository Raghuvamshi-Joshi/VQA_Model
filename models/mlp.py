...
def main():
    ...
    print("Get Questions,Answers,features...")
    ...
    print("Get input features, questions, answers... done")
    ...
    print("VGG features loaded...done")
    ...
    if args.language_only:
        model.add(Dense(args.num_hidden_units, input_dim=word_vec_dim, kernel_initializer="uniform"))
    else:
        model.add(Dense(args.num_hidden_units, input_dim=img_dim+word_vec_dim, kernel_initializer='uniform'))
    model.add(Activation(args.activation))
    if args.dropout>0:
        model.add(Dropout(args.dropout))
    for i in range(args.num_hidden_layers-1):
        model.add(Dense(args.num_hidden_units, kernel_initializer='uniform'))
        model.add(Activation(args.activation))
        if args.dropout>0:
            model.add(Dropout(args.dropout))
    ...
    print('Composing model file name')
    ...
    for k in range(args.num_epochs):
        ...
        for qu_batch,an_batch,im_batch in zip(grouper(questions_train, args.batch_size, fillvalue=questions_train[-1]),
            ...
            print("Batch 1 ...%d %d"%(len(qu_batch),len(an_batch)))
